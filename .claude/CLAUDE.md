...
Entry point: `/optimize [describe your model optimization goal]` → `commands/optimize.md` → `ml-optimizer:orchestrate` skill.
...
Phase 7: Workflow(phase-7-experiment) — Experiment loop (autonomous, pipelined):
         ...
         monitor → divergence polling folded into each experiment agent (a standalone monitor-agent dispatch is documented as a not-currently-wired-in pattern in phase-7-experiment-loop.md — no flag/user_choices key/code path invokes it today)
...
Phase 8: Workflow(phase-8-stacking) — Method stacking (when accumulated stacking_candidates is non-empty at Phase 7 exit):
         ...
         Per step: implement(merge) → review → experiment → analysis → [evolve] → [hp-tune]
...
/optimize → orchestrator-agent
  ...
  │   │  Reuses implement-agent + pr-review-toolkit:code-reviewer + pr-review-toolkit:silent-failure-hunter + pr-review-toolkit:pr-test-analyzer.
  ...
  │   │  hp_expand / narrow_space / regularization / stop; stuck protocol = a research
  ...
  │   │   ├─ monitor-agent (agentType, NOT currently wired into any workflow) → (no file output; in-run divergence polling folded into experiment-agent)
...
Cross-cutting outputs (managed by scripts or multiple agents):
  ...
  pipeline_state.py      → pipeline-state.json (phase, iteration, user_choices, consecutive_stop_count, stuck_protocol_triggered, baseline_checksum)
...
| `${CLAUDE_PLUGIN_ROOT}/scripts/goal_memory.py` | `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/goal_memory.py <exp_root> init-goals\|update-goals <updates_json>\|read-goals\|log-behavior <category> <json>\|query-behaviors [category]\|validate-output <agent> <json>\|summary\|sync-from-errors` — goal anchoring, behavioral memory, agent output validation, compact briefings |
...
- **Pre-loop**: user chooses Phase 4 option 1, 3, or 4 (all proceed to Phase 5 research before the experiment loop starts) — NOT option 5 ("Skip to Experiments"), which skips Phases 5-6 entirely
- **Mid-loop** (inside the Phase 7 workflow): when the analysis agent returns `decision: "method_proposal"` (or `pivot_type: "qualitative_change"`, which the workflow reroutes into `decision: "method_proposal"`), the workflow runs research + implement `agent()` calls (or a nested workflow)
...
- **Loop exit conditions**: The experiment loop is autonomous — runs until: (1) target metric achieved, (2) user stops, or (3) the phase-7 workflow judges approaches exhausted. When analysis advises stop, the workflow invokes the stuck protocol (research for fresh ideas), then decides on evidence: continue if research yields fresh non-dead-end proposals, exit to Phase 9 if genuinely out of directions (no new proposals, flat metric, empty agenda). No hardcoded stop-count threshold — `consecutive_stop_count` is one input to the judgment.
...
- **Research failure recovery**: Recovery is internal to a single research-agent dispatch inside the Phase 5 workflow — there is no separate orchestrator-side `source: "knowledge"` retry dispatch. The research skill runs WebSearch and alphaxiv in parallel and, for `source: "both"`, also draws on LLM training knowledge, so a web/alphaxiv failure degrades gracefully within that one dispatch (alphaxiv failure alone never triggers a fallback — WebSearch results suffice). If vetting still leaves zero proposals, the workflow writes a minimal `research-findings.md` and returns empty `proposals`, and the orchestrator continues HP-only. Each step is logged.
...
- **All-diverge recovery**: if all experiments in a batch diverge, the workflow flags this to the analysis-agent, which typically responds with a narrow_space/hp_expand pivot (e.g. shrinking the LR range) before recommending stop — not a scripted halved-LR batch.
...
- **Tabular ML adaptive timeout**: For non-iterative frameworks, experiment timeout is `fit_duration * (max_iters / profiling_iters) * 2`, capped at 4 hours (14400s), instead of the generic 6-hour pipeline-wide fallback used when no profiling data exists ("Experiment timeout" above).
- **Method stacking (Phase 8)**: After independent testing finds multiple methods that improve over baseline (no fixed count — `stacking_candidates` accumulate continuously across the whole Phase 7 run, from the analysis agent's own `stacking_candidates` field each batch plus the workflow's independent baseline-comparison harvest; the orchestrator enters Phase 8 automatically whenever that accumulated list is non-empty at loop exit, not gated by a specific decision/pivot_type on any one batch), the orchestrator sequentially merges them in descending improvement order. The strongest individual method's branch seeds the stack (no new branch is cut for it); each subsequent step creates `ml-opt/stack-<N>` (starting at N=2) by merging the next method into the current best stack. Clean merges proceed directly; conflicts are resolved by the implement-agent. Each merged branch is reviewed (`pr-review-toolkit:code-reviewer` + `pr-review-toolkit:silent-failure-hunter`, mirroring Phase 6) before the stacked experiment — a critical finding (e.g., a conflict resolution that dropped a NaN/CUDA guard) skips that method, since a merge-introduced silent failure would corrupt the metric. If a combination degrades, that method is skipped. After each successful step, the analysis agent assesses the result — if the stacked gain is less than the best individual method's gain (method interference), it recommends `code_evolution` and the evolve skill (`Skill("ml-optimizer:evolve")`) optimizes code-level interactions via ShinkaEvolve (5 generations, population 2), mirroring Phase 7 where analysis is always the decision-maker for evolution. If the evolved code improves over the pre-evolution stack, it becomes the new base; else discarded. Optional HP-tuning (one narrowed round) follows on the (potentially evolved) code when the combo shows >1% improvement. Stacking is resumable **within a session** via the `Workflow` runtime's `resumeFromRunId` only — there is no cross-session persistence of stacking progress: `pipeline-state.json` holds no `stacking` key, and a restart after Phase 8 was interrupted re-launches `phase-8-stacking.js` as a new run (recomputing `stacking_candidates` from the Phase 7 return) rather than resuming mid-stack. Requires git branch strategy — skipped for `file_backup` projects.
...
- **Statistical confidence assessment**: The analyze skill (Step 2.2) applies effect-size categories (Cohen's d: negligible/small/medium/large) and confidence labels (high/medium/low/preliminary) as guidelines from available data, not a rigid experiment-count gate — don't refuse to analyze with fewer experiments. Method attribution distinguishes whether improvements came from the code change, HP tuning, or their compound effect.

...
Phase 7: Experiment loop (autonomous, pipelined):
         ...
         review → Mid-pipeline review (async in background)
...
- **Async mid-pipeline review**: Mid-pipeline review runs in the background while the next experiment batch starts. Suggestions are applied one batch late — acceptable trade-off vs blocking the pipeline.

...
- **Reinforcement learning:** supported — RL hyperparameter tuning, environment-step budgets (`fixed_step_budget`), reward-collapse and shift-invariant divergence detection, sb3/rsl_rl log parsing, and multi-task evaluation across environments. Domain-randomization ranges are tunable as `<name>_center` / `<name>_width` pairs at `architecture` scope or above. Curriculum learning is orchestrated through the normal research → implement → tune path (proposed as a code change, branched, then its ramp parameters tuned), not as a bare hyperparameter. Multi-agent coordination remains out of scope.
- **Multi-seed ensembling:** seed *variance* is supported — experiments carry `random_seed`, and `result_analyzer.aggregate_replicates` groups runs that differ only by seed to report mean and spread. Combining several seeds' policies into one ensembled model at inference time is out of scope.

...
**Args (in):** `{ exp_root, project_root, primary_metric, lower_is_better, baseline_metric, scope_level, divergence_metric, divergence_lower_is_better, model_category, fixed_time_budget, fixed_epoch_budget, fixed_step_budget, stacking_candidates, remote }`
where `stacking_candidates: [{branch, improvement_pct}, ...]` comes from the phase-7 return. Pass `remote` whenever phase 7 had it — a stacked run measured on different hardware than the experiments it is compared against is not a valid comparison. `baseline_metric` (the baseline's primary-metric value) lets the workflow derive the seed method's absolute metric so the first accumulation is compared, not force-kept. `scope_level` gates the code_evolution/ShinkaEvolve repair (only runs at `"full"`, mirroring Phase 7). `fixed_step_budget`/`fixed_time_budget`/`fixed_epoch_budget` cap each stacked run the same way the baseline was (step budget takes precedence, for RL timestep-based training), so metrics stay comparable. `divergence_metric`/`divergence_lower_is_better`/`model_category` build the same divergenceClause Phase 7 uses, appended to every stacked experiment prompt — stacked runs get identical divergence handling to Phase 7 runs.
